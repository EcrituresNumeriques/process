...
#Split versions string
versions="${GET[version]}"
echo "<p>${versions}</p>"
IFS='-' read -ra VERSIONS <<< "${versions}"    #Convert string to array
# 
...
echo "<p>id : ${GET[id]}<br> Article : ${GET[article]}<br>" 
counter=0
for i in "${VERSIONS[@]}"; do
     counter=$((counter+1))
     echo "Chapter $counter: $i"
done


echo "Format : ${GET[format]}<br>Bibliographical style : ${GET[bibstyle]}<br>Processor : ${GET[processor]}</p>"
# 
